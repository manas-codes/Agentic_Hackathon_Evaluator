...
# TLS terminates at the platform, so the app is reached over plain HTTP and the
# request scheme reads as "http". Without this, redirect URLs generated by the
# app come back as http:// and a browser on an https:// page refuses to follow
# them. ProxyHeaders restores the real scheme from X-Forwarded-Proto.
if _SECURE_COOKIES:
